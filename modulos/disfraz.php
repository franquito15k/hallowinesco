...
if ($_GET['accion'] == 'guardar_insertar') {
    ...
    if (mysqli_num_rows($sql) != 0) {
        echo "<script> alert('El disfraz ya esta cargado en la Base de datos');</script>";
    } else {
        //procesar la foto
        ...
        $sql = mysqli_query($con, $sql);
        if (mysqli_error($con)) {
            echo "<script> alert('Error al insertar el disfraz en la bd);</script>";
        } else {
            echo "<script> alert('Disfraz cargado con exito. :D');</script>";
        }
    }
}
...
if ($_GET['accion'] == 'guardar_editar') {
    ...
    $sql = mysqli_query($con, $sql);
    if (!mysqli_error($con))
        echo "<script> alert('Disfraz editado con exito :D');</script>";
    else
...
if ($_GET['accion'] == 'guardar_eliminar') {
    ...
    $sql = mysqli_query($con, $sql);
    if (!mysqli_error($con))
        echo "<script> alert('Disfraz eliminado con exito :O');</script>";
    else
...
<section id="admin" class="section">
    ...
    if ($_GET['accion'] == 'editar') {
        $url = 'index.php?modulo=disfraz&accion=guardar_editar&id=' . $_GET['id'];
        ...
        $url = 'index.php?modulo=disfraz&accion=guardar_insertar';
    ...
    <table border="1" style="width: 100%;">
        ...
        ?>
                <tr>
                    ...
                    <td>
                        <a href="index.php?modulo=disfraz&accion=editar&id=<?php echo $r['id']; ?>">Editar</a>
                        -
                        <a href="javascript:if(confirm('Desea eliminar el registro?')) window.location='index.php?modulo=disfraz&accion=guardar_eliminar&id=<?php echo $r['id']; ?>'">Eliminar</a>
                    </td>
                </tr>
        <?php
            }
